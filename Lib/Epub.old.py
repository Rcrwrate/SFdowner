...
class Epub():
    def __init__(self, name, author, update_time) -> None:
        self.name = name
        self.author = author
    ...
    def forder_init(self):
        mkdir(".tmp")
        mkdir(os.path.join(".tmp", self.name))
        mkdir(os.path.join(".tmp", self.name, "META-INF"))
        with open(os.path.join(".tmp", self.name, "META-INF", "container.xml"), "w", encoding="utf-8") as f:
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
    <rootfiles>
        ...
        with open(os.path.join(".tmp", self.name, "mimetype"), "w", encoding="utf-8") as f:
            f.write('''application/epub+zip''')
        mkdir(os.path.join(".tmp", self.name, "OEBPS"))
        mkdir(os.path.join(".tmp", self.name, "OEBPS", "Images"))
        mkdir(os.path.join(".tmp", self.name, "OEBPS", "Styles"))
        mkdir(os.path.join(".tmp", self.name, "OEBPS", "Text"))
    ...
    def download(self, url: list):
        ...
        for i in url:
            try:
                ...
                if inputs == "Y":
                    print(
                        f'''[TIPS]:您需要手动下载该文件置于.tmp/{self.name}/OEBPS/Images/{i.split("/")[-1]}''')
                    self.pics.append(i)
                    fin.append(i)
        return fin
